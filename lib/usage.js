...
export function normalizeUsage(raw) {
  ...
  const hasInputDetails = Object.prototype.hasOwnProperty.call(raw, 'input_tokens_details');
  const inputDetails = raw.input_tokens_details != null && typeof raw.input_tokens_details === 'object'
    ? raw.input_tokens_details
    : {};
  const cached = inputDetails.cached_tokens ?? 0;
  const cacheWriteDetail = inputDetails.cache_write_tokens ?? 0;
  ...
  const cacheWrite = c5 + c1h > 0
    ? c5 + c1h
    : (raw.cache_creation_input_tokens ?? (hasInputDetails ? cacheWriteDetail : 0));
  const cacheRead = raw.cache_read_input_tokens ?? (hasInputDetails ? cached : 0);
  return {
    input_tokens: hasInputDetails
      ? (raw.input_tokens ?? 0) - cacheRead - cacheWrite
      : (raw.input_tokens ?? 0),
    ...
    output_tokens_details: {
      thinking_tokens: raw.output_tokens_details?.thinking_tokens
