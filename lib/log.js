...
// Returns the ANSI escape for a model name's value, hashed via the same
// djb2-over-first-8-chars scheme used for session brackets. The palette is
// shared with SESSION_COLORS so the same model always renders the same color
// and the bracket/value color roles don't clash.
...
export function __colorEscapeForModel(model) {
  const colorCode = SESSION_COLORS[hashModel(model) % SESSION_COLORS.length];
  return colorCode < 38
    ? `\x1b[${colorCode}m`
    : `\x1b[38;5;${colorCode}m`;
}
...
export function logRes(id, fields) {
  ...
  if (fields.durationMs != null) {
    const dur = __abbrev(fields.durationMs);
...
// it is not computed from the others.
...
function formatUsage(usage) {
  ...
  push('write', usage.cache_creation_input_tokens, 'write');
  const segments = [];
