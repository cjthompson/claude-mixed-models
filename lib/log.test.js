...
// Thinking tokens nonzero → extra `reasoning: N` segment after `out:`, before
...
// segment).
test('logRes: output_tokens_details.thinking_tokens > 0 shows a reasoning segment', () => {
  stubNow('01:23:46');
  const captured = captureLog(() => logRes('id', {
    upstream: 'anthropic',
    status: 200,
  ...
  const cwEsc = '\x1b[33m';
  const totalEsc = '\x1b[1;37m';
  // reasoning: 83 between out: and cache read:, in the bold-white color.
  ...
  assert.deepEqual(captured, [
    ...
    ` [in: ${inEsc}2${RESET} | out: ${outEsc}1k${RESET} | reasoning: ${totalEsc}83${RESET}` +
